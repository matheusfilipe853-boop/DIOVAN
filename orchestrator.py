...
class Orchestrator:
    def __init__(self):
        ...
        # Inicializa interfaces para cada nível
        self.llm_fast    = LLMInterface(model=os.getenv("DIOVAN_MODEL_FAST",    "gemma2:2b"))
        self.llm_normal  = LLMInterface(model=os.getenv("DIOVAN_MODEL_NORMAL",  "llama3.2"))
        self.llm_complex = LLMInterface(model=os.getenv("DIOVAN_MODEL_COMPLEX", "diovan"))

    def _select_model(self, text: str) -> tuple:
        """Seleciona o modelo baseado na complexidade da pergunta"""
        text_lower = text.lower().strip()
        word_count = len(text_lower.split())

        # Nível 1 — simples
        if word_count <= 3 or any(t in text_lower for t in SIMPLE_TRIGGERS):
            return self.llm_fast, "⚡ Nível 1"

        # Nível 3 — complexo
        if word_count > 20 or any(t in text_lower for t in COMPLEX_TRIGGERS):
            return self.llm_complex, "🧠 Nível 3"

        # Nível 2 — padrão
        return self.llm_normal, "💡 Nível 2"
...
Responda com base nesse contexto real.
Não invente informações que não estejam no contexto.
Responda sempre em português brasileiro.
            """.strip()

        self.history.append({"role": "user", "content": user_input})
        response = llm.process(enriched_input, self.history)
